...
- **分支索引与 O(history)。** 分段缓存结构、`base_branch_id/base_seq` 链与「取新前缀段中最新 compaction 为复制边界」的算法按上游保留；T12 起 compaction 条目会形成复制边界，尚未压缩的长前缀仍可能复制 O(history)，这是 ADR-0005 接受的限制。查询计划（上游用 `EXPLAIN QUERY PLAN` 断言 `ix_be_seq`/CROSS JOIN）未移植：那是 schema 级约定，本票以公开读行为验证「无遗漏、无重复」。
...
- **T12 无 schema 变更。** compaction entry 复用 `entries.payload`，以 camelCase 保存 summary、retained tail、tokens before、details、usage 与 hook 来源；结构 operation 的 preparation/state 复用 scalar values，逐请求 usage 复用 ledger。SQLite 重开测试覆盖 effect-pending 摘要以新 attempt 恢复。
