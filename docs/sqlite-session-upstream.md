...
- **显式持久化编解码。** 上游的 TypeScript 对象本身可 JSON 化，消息与 usage 直接落盘；Python 存的是 dataclass，因此新增 `omh/agent/session/codec.py`（`encode_message`/`decode_message`/`encode_usage`/`decode_usage`）。落盘键名沿用上游 camelCase、可选字段缺失即省略，便于与格式 4 对照；读取时校验 payload 形状，坏数据抛 `ValueError` 而不是静默构造半成品消息。T05 起，message entry 的 `terminate: true` 与 message 一起存入现有 `payload`，无需改变 SQLite schema。
