...
class _MemorySessionFacade:
    ...
    async def mutate[T](self, mutation: SessionMutationCallback[T], context: Context) -> T:
        self._assert_open()
        return await self._session.mutate(mutation, context)

    async def get_entries(self, ids: list[str], context: Context) -> dict[str, Entry]:
        self._assert_open()
        return await self._session.get_entries(ids, context)

    async def get_entry(self, entry_id: str, context: Context) -> Entry | None:
        self._assert_open()
        return await self._session.get_entry(entry_id, context)

    async def get_value[T](self, address: Value[T], context: Context) -> StoredValue[T] | None:
        self._assert_open()
        return await self._session.get_value(address, context)

    async def scan_values[T](self, prefix: Value[T], context: Context) -> list[StoredValue[T]]:
        self._assert_open()
        return await self._session.scan_values(prefix, context)

    async def read_list[T](
        self, address: ValueList[T], options: ListReadOptions | None, context: Context
    ) -> list[ListElement[T]]:
        self._assert_open()
        return await self._session.read_list(address, options, context)

    async def scan_branch(self, query: StorageBranchScan, context: Context) -> list[Entry]:
        self._assert_open()
        return await self._session.scan_branch(query, context)

    async def get_stats(self, context: Context) -> SessionStats:
        self._assert_open()
        return await self._session.get_stats(context)

    async def scan_usage(self, query: UsageScan, context: Context) -> list[UsageRow]:
        self._assert_open()
        return await self._session.scan_usage(query, context)

    async def find_entries(self, query: EntryQuery | None, context: Context) -> list[Entry]:
        self._assert_open()
        return await self._session.find_entries(query, context)

    async def find_entry(self, query: EntryQuery | None, context: Context) -> Entry | None:
        self._assert_open()
        return await self._session.find_entry(query, context)

    async def branch(self, name: str, context: Context) -> Branch | None:
        self._assert_open()
        branch = await self._session.branch(name, context)
...
class MemorySessionRepo:
    ...
    async def create(self, options: SessionCreateOptions, context: Context) -> _MemorySessionFacade:
        del context
        self._assert_open()
    ...
    async def open(self, metadata: SessionMetadata, context: Context) -> _MemorySessionFacade:
        del context
        self._assert_open()
    ...
    def _open_record(self, record: _MemorySessionRecord) -> _MemorySessionFacade:
