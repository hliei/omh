...
-- A SQLite database file is a session container. This backend creates one file
-- per Session, but the schema supports any number of Sessions per file by
-- scoping every durable row with session_id.
-- Authoritative durable state is entries + scalar_values + list_values +
-- usage_ledger; branch_* and the stats columns on sessions are maintained
-- projections/caches.
...
-- namespace and ordered parent insertion without a preflight pass.
